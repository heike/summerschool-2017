...
- 2013 telephone survey with approx 500k responses run by the Center for Disease Control (CDC)
...
- The file `states.health.stats` contains information of the percentage of children in each state without healthcare coverage.
...
qplot(long, lat, geom="polygon", data = states.map, group = group, fill = avg.drnk) + 
  ...
  ggtitle("Map of Average Number of Alcoholic Beverages Consumed Per Day by State") +
  theme(legend.position="right") +
  geom_point(aes(x = long, y=lat), data = us.cities, inherit.aes = FALSE) +
  ggrepel::geom_label_repel(aes(x = long, y = lat, label = name), data = us.cities[us.cities$pop>100000,], 
                            inherit.aes = FALSE)
```
